...
const HTML = {
	ADD_BUTTON: `
		<a role="button" class="admin-policy-catalog-add">분석 활용 데이터 추가</a>
	`,
	DELETE_BUTTON: `
		<a role="button" class="admin-policy-catalog-delete">삭제</a>
	`,
	DATE_CHANGER: (v) => `
		<select class="admin-policy-catalog-date">
			${v.mta_dates.map(d => `<option value="${d}" ${d == v.pm_date ? 'selected' : ''}>${d}</option>`).join('')}
		</select>
	`,
	USE_STAT_SWITCH: (v) => `
		<label class="admin-policy-catalog-use-stat switch">
	...
	SEQUENCE_CHANGER: `
		<div class="u-text-align-center">
			<a class="bx bx-chevron-up-circle admin-policy-catalog-up"></a>
			<a class="bx bx-chevron-down-circle admin-policy-catalog-down"></a>
		</div>
	`,
	GRID: `
		<table class="admin-policy-catalog-grid">
			<thead>
				<tr>
	...
	`,
	DIALOG: `
		<table class="admin-policy-catalog-dialog-grid">
			<thead>
				<tr>
					<th data-field="mta_nm">메타데이터명</th>
...
class AdminPolicyCatalogManager {
	constructor(opts = {}) {
	...
	init() {
		...
		this.$element.on('click', '.admin-policy-catalog-add', onAddButtonClick.bind(this));
		this.$element.on('click', '.admin-policy-catalog-delete', onDeleteButtonClick.bind(this));
		this.$element.on('click', '.admin-policy-catalog-use-stat', onUseStatSwitchClick.bind(this));
		this.$element.on('click', '.admin-policy-catalog-up, .admin-policy-catalog-down', onSequenceChangerClick.bind(this));
		this.$element.on('change', '.admin-policy-catalog-date', onDateSelectChange.bind(this));
		...
		async function onSequenceChangerClick(e) {
			...
			const isUp = $this.is('.admin-policy-catalog-up');
			const isDown = $this.is('.admin-policy-catalog-down');
		...
		function onManagerGridChange(e) {
			const getGridButton = () => this.$element.find('.admin-policy-catalog-add');
	...
	initGrid() {
		...
		const $grid = this.$element.find('.admin-policy-catalog-grid');
...
export default AdminPolicyCatalogManager;