...
const HTML = {
	ADD_BUTTON: `
		<a role="button" class="admin-indicator-catalog-add">분석 활용 데이터 추가</a>
	`,
	DELETE_BUTTON: `
		<a role="button" class="admin-indicator-catalog-delete">삭제</a>
	`,
	DATE_CHANGER: (v) => `
		<select class="admin-indicator-catalog-date">
			${v.mta_dates.map(d => `<option value="${d}" ${d == v.pm_date ? 'selected' : ''}>${d}</option>`).join('')}
		</select>
	`,
	USE_STAT_SWITCH: (v) => `
		<label class="admin-indicator-catalog-use-stat switch">
	...
	SEQUENCE_CHANGER: `
		<div class="u-text-align-center">
			<a class="bx bx-chevron-up-circle admin-indicator-catalog-up"></a>
			<a class="bx bx-chevron-down-circle admin-indicator-catalog-down"></a>
		</div>
	`,
	GRID: `
		<table class="admin-indicator-catalog-grid">
			<thead>
				<tr>
	...
	`,
	DIALOG: `
		<table class="admin-indicator-catalog-dialog-grid">
			<thead>
				<tr>
					<th data-field="mta_nm">메타데이터명</th>
...
class AdminIndicatorCatalogManager {
	constructor(opts = {}) {
	...
	init() {
		...
		this.$element.on('click', '.admin-indicator-catalog-add', onAddButtonClick.bind(this));
		this.$element.on('click', '.admin-indicator-catalog-delete', onDeleteButtonClick.bind(this));
		this.$element.on('click', '.admin-indicator-catalog-use-stat', onUseStatSwitchClick.bind(this));
		this.$element.on('click', '.admin-indicator-catalog-up, .admin-indicator-catalog-down', onSequenceChangerClick.bind(this));
		this.$element.on('change', '.admin-indicator-catalog-date', onDateSelectChange.bind(this));
		...
		async function onSequenceChangerClick(e) {
			...
			const isUp = $this.is('.admin-indicator-catalog-up');
			const isDown = $this.is('.admin-indicator-catalog-down');
		...
		function onManagerGridChange(e) {
			const getGridButton = () => this.$element.find('.admin-indicator-catalog-add');
	...
	initGrid() {
		...
		const $grid = this.$element.find('.admin-indicator-catalog-grid');
	...
	initDialog() {
		...
		const $dialog = $('<div class="admin-indicator-catalog-dialog"/>');
		$dialog.appendTo($('body'));
		$dialog.html(HTML.DIALOG);
		$dialog.kendoDialog($.extend(true, {}, Constant.KENDO_DEFAULT_DIALOG_OPTION, kendoDialogOption, this.kendoDialogOption));

		const $dialogGrid = $dialog.find('.admin-indicator-catalog-dialog-grid');
...
export default AdminIndicatorCatalogManager;