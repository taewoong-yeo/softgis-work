...
{
	//var longitude = 204992;
	//var latitude = 445618;
	
	var longitude = x;
	var latitude = y;
	
	var TM_Code 		= new CS(csList.GRS80_TM);				//TM (�ߺα�)
	var WGS84_Code 	= new CS(csList.GOOGLE_WGS84);		//WSG84
		
	var p = new PT(longitude, latitude);
	cs_transform(TM_Code, WGS84_Code, p);
	
	return p;
}

function WGS84toTM(longitude, latiude)
{
	...
	var m_latitude 	= latiude;
	
	var p = new PT(m_longitude, m_latitude);
	cs_transform(WGS84_Code, TM_Code, p)
...
function KATECHtoWGS84(x, y)
{
	...
	var KATECH_Code = new CS(csList.TM128_katech_3param);	//KATECH 좌표
	var WGS84_Code 	= new CS(csList.GOOGLE_WGS84);			//WSG84
		
	var p = new PT(longitude, latitude);
	cs_transform(KATECH_Code, WGS84_Code, p);
	
	return p;
}

function WGS84toKATECH(longitude, latiude)
{
	...
	var p = new PT(m_longitude, m_latitude);
	cs_transform(WGS84_Code, KATECH_Code, p)
	
	return p;
}

function KATECHtoTM(x, y)
{
	var longitude = x;
	var latitude = y;
	
	var KATECH_Code = new CS(csList.TM128_katech_3param);	//KATECH 좌표
	var TM_Code 	= new CS(csList.KOREA_CENTER_TM_3param);			//WSG84
		
	var p = new PT(longitude, latitude);
	cs_transform(KATECH_Code, TM_Code, p);
	
	return p;
}

function TMtoKATECH(x, y)
{
	var KATECH_Code = new CS(csList.TM128_katech_3param);		//KATECH 좌표
	var TM_Code 	= new CS(csList.KOREA_CENTER_TM_3param);	//WSG84
	...
	var p = new PT(m_longitude, m_latitude);
	cs_transform(TM_Code, KATECH_Code, p)
