	...
	function getChart(){
		$.ajax({
			...
			success: function(data){
				...
				vis.vis_series = "Y";
				let resultData1 = dataTransSeries(data.result.yearWasteCount, 'series');
				setString(data.result.yearWasteCount, vis, "yearWasteCount");
				createChart(vis, resultData1);
				
				// 지역별 폐기물 현황
				vis.vis_nm = "지역별 폐기물 현황";
				vis.trgt_div = "areaWasteCount";
				let resultData2 = dataTransSeries(data.result.areaWasteCount, 'series');
				...
				let resultData3 = dataTransSeries(data.result.yearWasteIncrease, 'series');
				setString(data.result.yearWasteIncrease, vis, "yearWasteIncrease");
				resultData3.format = '{point.y}%'; //.f 소수점자리
				createChart(vis, resultData3);
				
				// 지역별 인구수
				if(data.result.areaPopltnCount != undefined) {
					vis.vis_nm = "지역별 인구수";
					let resultData4 = dataTrans(data.result.areaPopltnCount, 'default');
				...
				if(data.result.resultData != undefined) {
					...
					let resultData5 = dataTrans(data.result.resultData, 'multiSeries');
