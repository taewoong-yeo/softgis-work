...
//			]
	...
	const validator = new Validator({
		...
		start_dt: { function: chkDateDontCare },
		end_dt: { function: chkStartDt },
//		map_sel_cd: { required: true },
//		map_sel_option: { required: true },
		answer_cd: { required: true },
...
//			img_file: '썸네일'
	...
	//취소 버튼 클릭
	$(".cancleButton").on("click", () => location.href=Constant.CONTEXT_PATH + "/cmmntyMap/cmmnty-list.do" );
...
//			e.preventDefault();
	...
	async function loadMarker() {
		...
		for(const i in markerList.marker) {
			...
			const $item = $('<div class="cmmntyMap-style-marker-item"><img src="'+Constant.CONTEXT_PATH + '/assets/images/marker/' + marker+'"/></div>');
//			$item.attr('src', Constant.CONTEXT_PATH + '/assets/images/marker/' + marker);
...
//		if($this.siblings(".active").length == 5){
...
export default CmmntyFormRoute;